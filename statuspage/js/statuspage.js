...
document.addEventListener(
  ...
  function() {
    ...
    checkup.storage.getChecksWithin(
      checkup.config.timeframe,
      processNewCheckFile,
      allCheckFilesLoaded
    );
...
function processNewCheckFile(json, filename) {
  ...
  var process = function(result) {
    ...
    else checkup.groupedResults[result.timestamp].push(result);

    if (!checkup.results[result.endpoint])
      checkup.results[result.endpoint] = [result];
    else checkup.results[result.endpoint].push(result);

    var chart =
      checkup.charts[result.endpoint] || checkup.makeChart(result.title);
    ...
    chart.series.med.push({
      timestamp: ts,
      rtt: Math.round(result.stats.median)
    });
    ...
    if (!checkup.lastResultTs || ts > checkup.lastResultTs) {
      ...
      checkup.dom.lastcheck.innerHTML =
        checkup.makeTimeTag(checkup.lastResultTs) + " ago";
    }
    return chart;
  };
  ...
  values(checkup.charts).forEach(function(chart) {
    values(chart.series).forEach(function(series) {
      series.sort(byTimestamp);
    });
  });

  if (checkup.domReady) makeGraphs();
}

function allCheckFilesLoaded(numChecksLoaded, numResultsLoaded) {
  // Sort the result lists
  checkup.orderedResults.sort(function(a, b) {
    return a.timestamp - b.timestamp;
  });
  for (var endpoint in checkup.results)
    checkup.results[endpoint].sort(function(a, b) {
      return a.timestamp - b.timestamp;
    });
  ...
  for (
    var i = checkup.orderedResults.length - numResultsLoaded;
    i < checkup.orderedResults.length;
    i++
  ) {
  ...
  for (var i = 0; i < newEvents.length; i++) {
    ...
    var imgFile = "ok.png",
      imgWidth = 15,
      imgHeight = 15; // the different icons look smaller/larger because of their shape
    if (e.status == "down") {
    ...
    if (e.message) {
      ...
      evtElem.innerHTML =
        ...
        checkup.makeTimeTag(e.result.timestamp * 1e-6) +
        " ago</div>";
      ...
      evtElem.innerHTML =
        '<span class="time">' +
        renderTime(e.result.timestamp) +
        "</span> " +
        e.result.title +
        " " +
        e.status;
    }
    checkup.dom.timeline.insertBefore(
      evtElem,
      checkup.dom.timeline.childNodes[0]
    );
  }
  ...
  for (var endpoint in checkup.results) {
    ...
    var lastResult =
      checkup.results[endpoint][checkup.results[endpoint].length - 1];
    if (lastResult) {
      if (lastResult.down) overall = "down";
      else if (lastResult.degraded) overall = "degraded";
    }
  }
  ...
  for (var key in checkup.charts) {
    ...
    for (var k = 1; k < checkup.charts[key].results.length; k++) {
      var timeDiff = Math.abs(
        checkup.charts[key].results[k].timestamp -
          checkup.charts[key].results[k - 1].timestamp
      );
...
function makeGraphs() {
  checkup.dom.timeframe.innerHTML = checkup.formatDuration(
    checkup.config.timeframe
  );
...
function makeGraph(chart, endpoint) {
  ...
  chart.xScale.domain([
    d3.min(chart.data, function(c) {
      return d3.min(c, function(d) {
        return d.timestamp;
      });
    }),
    d3.max(chart.data, function(c) {
  ...
  chart.lines
    ...
    .duration(checkup.animDuration)
    .attr("d", chart.line);
  chart.events.transition().duration(checkup.animDuration);
  ...
  chart.events
    ...
    .attr("xlink:href", function(d, i) {
      return "images/" + d.imgFile;
    })
    .attr("x", function(d, i) {
      return chart.xScale(d.timestamp) - d.imgWidth / 2;
    })
    .attr("y", function(d, i) {
      return chart.yScale(d.rtt) - d.imgHeight / 2;
    })
    .attr("data-eventid", function(d, i) {
      return d.eventid;
    })
    .attr("class", function(d, i) {
      return "event-item event-id-" + d.eventid;
    })
    .on("mouseover", highlightSameEvent)
    .on("mouseout", unhighlightSameEvent);

  // exit any old data
  chart.lines.exit().remove();
}
...
function renderChart(chart) {
  ...
  if (document.getElementsByClassName("chart-container").length == 0) {
    ...
    if (tooWide) tooWide.className = "chart-container chart-50";
  }
  ...
  el2b.setAttribute("href", chart.endpoint);
  ...
  chart.line = d3.svg
    .line()
    .x(function(d) {
      return chart.xScale(d.timestamp);
    })
    .y(function(d) {
      return chart.yScale(d.rtt);
    })
    .interpolate("step-after"); // linear, monotone, or basis

  chart.lineGroup = chart.svg.append("g").attr("class", "lines");
  ...
  var mousemove = function() {
    var x0 = chart.xScale.invert(d3.mouse(this)[0]),
      ...
      d =
        d0 && d1 ? (x0 - d0.timestamp > d1.timestamp - x0 ? d1 : d0) : d0 || d1;
    var xloc = chart.xScale(d.timestamp);
    focus.attr(
      "transform",
      "translate(" + xloc + "," + chart.yScale(d.rtt) + ")"
    );
    if (xloc > overlay.width.animVal.value - 50)
      text.attr("transform", "translate(-60, 10)");
    else text.attr("transform", "translate(0, 10)");
  ...
  chart.eventGroup = chart.svg.append("g").attr("class", "events");
}

function highlightSameEvent() {
  var elems = document.querySelectorAll(
    ".event-item:not(.event-id-" + this.getAttribute("data-eventid") + ")"
  );
